(function($) {
    ...
    var hoverEffect = function(opts) {
        var vertex = `
			...
			void main() {
			  vUv = uv;
			  gl_Position = sermonsionMatrix * modelViewMatrix * vec4( position, 1.0 );
			}
		`;
    ...
    var sermons_grid = function($scope, $) {
        var sermons_grid_options = $scope.find(".awaiken-sermons-widget").data("config");

        if (sermons_grid_options && sermons_grid_options.show_filter_bar === "yes") {
            var $portfolio = $("#awaiken-portfolio-" + sermons_grid_options.id);
            var $grid = $portfolio.find(".awaiken-portfolio-grid");
            var $portfolio_filter = $portfolio.find(".awaiken-portfolio-grid__filters");

            // Ensure the grid and images exist before initializing
            if ($grid.length && $grid.find("img").length) {
                var isRTL = sermons_grid_options.rtl === true || sermons_grid_options.rtl === "true";
                $grid.imagesLoaded(function() {
                    $grid.isotope({
                        itemSelector: ".awaiken-portfolio-grid-item",
                        layoutMode: "masonry",
                        isOriginLeft: !isRTL,
                        masonry: {
                            gutter: parseInt(sermons_grid_options.item_spacing) || 0,
                        },
                    });
    ...
    var ministries_grid = function($scope, $) {
        var ministries_grid_options = $scope.find(".awaiken-ministries-widget").data("config");

        if (ministries_grid_options && ministries_grid_options.show_filter_bar === "yes") {
            var $portfolio = $("#awaiken-portfolio-" + ministries_grid_options.id);
            var $grid = $portfolio.find(".awaiken-portfolio-grid");
            var $portfolio_filter = $portfolio.find(".awaiken-portfolio-grid__filters");

            // Ensure the grid and images exist before initializing
            if ($grid.length && $grid.find("img").length) {
                var isRTL = ministries_grid_options.rtl === true || ministries_grid_options.rtl === "true";
                $grid.imagesLoaded(function() {
                    $grid.isotope({
                        itemSelector: ".awaiken-portfolio-grid-item",
                        layoutMode: "masonry",
                        isOriginLeft: !isRTL,
                        masonry: {
                            gutter: parseInt(ministries_grid_options.item_spacing) || 0,
                        },
                    });
    ...
    $window.on("elementor/frontend/init", function() {
        elementorFrontend.hooks.addAction("frontend/element_ready/emanu-sermons-grid.default", sermons_grid);
        elementorFrontend.hooks.addAction("frontend/element_ready/emanu-ministries-grid.default", ministries_grid);
